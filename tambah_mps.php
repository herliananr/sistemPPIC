...
<?php
    ...
    if (isset($_POST["submit"])) {
        ...
        if ($jumlah_data >= 1 ) {
        ...
        else if ($jumlah_qty_mps<$inputQty) {
            $penanda="iya";
            ...
            $pesan .= "Mohon untuk mengisi ID MPS. ";
        }
        ...
        if ($pesan==="") {
            ...
            $query = "insert into mps values ('$inputIDMPS', '$inputIDPO','$inputIDProduk','$inputNamaProduk','$inputUOM',
            '$inputTanggalMulaisatu','$inputTanggalSelesaisatu','$inputQtysatu',
            '$inputTanggalMulaidua', '$inputTanggalSelesaidua','$inputQtydua',
            '$inputTanggalMulaitiga', '$inputTanggalSelesaitiga', '$inputQtytiga',
            '$inputTanggalMulaiempat', '$inputTanggalSelesaiempat', '$inputQtyempat')";
            $hasil = mysqli_query($link, $query);

            if ($hasil) {
                $pesandikirim .="Produk dengan ID $inputIDMPS berhasil ditambahkan.";
        ...
        if ($diterima_id !=="") {
            include("layout/koneksi.php");
            //deklarasi untuk mps
            $query="select max(id_mps) as id_mps_terbesar from mps";
            $result = mysqli_query($link, $query);
            $hasil = mysqli_fetch_array($result);
            $idmps = $hasil['id_mps_terbesar'];
            $potonganurutan = (int) substr($idmps,3,4);
            $potonganurutan++;

            $hurufdepan = "MPS";
            $inputIDMPS = $hurufdepan. sprintf("%04s", $potonganurutan);

            
            $inputIDPO=$diterima_id;
...
<body id="page-top">
                ...
                <div class="container-fluid">
                    ...
                    <div class="card shadow mb-4">
                        <div class="card-header py-3">
                            <h6 class="m-0 font-weight-bold text-primary">Tambah Master Production Schedule</h6>
                        </div>
                        <div class="card-body">
                            <form action="tambah_mps.php" class="col-10 offset-1" method="post">
                            ...
                            <div class="form-group row">
                                    ...
                                    <div class="col-sm-10">
                                        ...
                                        <?php
                                           if ($penanda=="iya") {
                                                ...
                                                $jsArray .= "dtP['" . $inputIDProduk . "'] = {namaproduk:'" . addslashes($inputNamaProduk). "',uomproduk:'".addslashes($inputUOM)."',
                                                    qtyproduk:'".addslashes($inputQty)."'};\n";
                                            }
                                            else if ($penanda==""){
                                                ...
                                                $query = "select id_produk, nama_produk, uom, qty from po where id_po='$inputIDPO' and (id_produk, nama_produk, uom, qty ) not in (select id_produk, nama_produk, uom, qty from mps where id_po='$inputIDPO')";
                                                $jsArray= "var dtP = new Array();\n";
                                                ...
                                                while ($hasil=mysqli_fetch_array($result)) {
                                                    ...
                                                    $jsArray .= "dtP['" . $hasil['id_produk'] . "'] = {namaproduk:'" . addslashes($hasil['nama_produk']) . "',uomproduk:'".addslashes($hasil['uom'])."',
                                                                qtyproduk:'".addslashes($hasil['qty'])."'};\n";
                                                }
                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col-sm-10">
                                        <input type="text" class="form-control" name="inputQty" id="inputQty" readonly 
                                            <?php
    ...
    <!--Ambil nilai mps untuk dimasukkan kedalam textbox-->
    <script>
        <?php echo $jsArray?> 
        ...
        function ubahProduk(inputIDProduk){
            ...
            document.getElementById('inputQty').value = dtP[inputIDProduk].qtyproduk;
        }        
